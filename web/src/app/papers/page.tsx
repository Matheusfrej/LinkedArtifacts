...
export default function Page() {
  ...
  const [pendingFilter, setPendingFilter] =
    useState<ArtifactFilterValue | null>(null)
  ...
  const handleFilterChange = (
    newFilters: ArtifactFilterValue[],
    ...
    return (
      <div className="py-12 space-y-4">
        <div className="p-6 rounded-xl border border-red-500/20 bg-red-500/10 text-red-600 dark:text-red-400">
          <h2 className="font-semibold text-base mb-1">Error loading papers</h2>
    ...
    <div className="py-4 sm:py-8">
      ...
      <div className="flex flex-col gap-3 mb-6">
        <div className="flex flex-row items-center justify-between gap-3 border-b border-border/40 pb-3">
          <div className="text-xs sm:text-sm text-muted-foreground font-medium min-h-[20px] flex items-center">
            {isTransitioning ? (
              <span className="inline-flex items-center gap-1.5 text-muted-foreground/80 animate-pulse font-medium">
                <Loader2 className="w-3.5 h-3.5 animate-spin text-primary shrink-0" />
                <span>Updating papers...</span>
              </span>
            ) : papers.length > 0 ? (
              <span>
      ...
      {/* Results List: Skeleton during search/filter/sort or actual cards */}
      {isTransitioning ? (
        <PaperListItemsSkeleton
